...
python3 test_runner.py 30 csv  > fib_run.csv

python3 test_runner.py 30 > fib_table.md
"""
...
COMMON_ARG_FORMAT = "./fib.out {n} {algo} 0"
FORMAT = "markdown"
TIMEOUT = 60
COMMON_ARG_PYTHON = "python3 fib.py --algo {algo} {n}"  # using pypy3 as it is faster than python
...
def build_row(n: int) -> str:
    ...
    Args:
        n (int): the row to build in the fibonacci
...
if __name__ == "__main__":
    _n = 10000 if len(sys.argv) < 2 else int(sys.argv[1])
    if len(sys.argv) == 3:
        FORMAT = "csv"
    main(_n)
